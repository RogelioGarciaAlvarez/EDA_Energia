...
#Muestra tantos diagramas de barras independientes como columnas se indiquen a la funcion
def multiples_categoricas(df, columnas_categoricas, relativa=False, mostrar_valores=False, giro = 45):
...
#Muestra tantos diagramas de caja independientes como columnas se indiquen a la funcion
def multiples_boxplot(df, columns, dim_matriz_visual = 2):
    ...
    for i, column in enumerate(columns):
        if df[column].dtype in ['int64', 'float64']:
            ...
            axes[i].set_title(f"Diagrama de caja de {column}")
...
#Muestra tantos histrogramas, funcion de densidad y diagramas de cajas independientes como columnas se indiquen a la funcion
def multiples_combinados(df, columns, whisker_width=1.5, bins = None):
    num_cols = len(columns)
    if num_cols:
        ...
        for i, column in enumerate(columns):
            if df[column].dtype in ['int64', 'float64']:
                ...
                if num_cols > 1:
                    axes[i,1].set_title(f'Diagrama de caja de {column}')
                else:
                    axes[1].set_title(f'Diagrama de caja de {column}')

        plt.tight_layout()
        plt.show()

#Muestra tantas series temporales independientes como columnas se indiquen a la funcion
def multiples_series_temporales(df, lista_columnas_numericas):
    #Se calcula el número de filas necesarias
    n_rows = (len(lista_columnas_numericas) + 1) // 2

    #Se crean los subplots
    fig, axs = plt.subplots(n_rows, 2, figsize=(12, n_rows * 4))

    #Se convierte cualquier array en un array unidimensional
    axs = axs.flatten()

    #Se itera sobre cada elemento de la lista para crear los graficos
    for i, columna in enumerate(lista_columnas_numericas):
        #Se crea la serie temporal
        sns.lineplot(data=df[columna], ax=axs[i])
        
        #Se pone el título y etiquetas
        axs[i].set_title(f"Serie temporal de {columna}")
        axs[i].set_xlabel("Índice")
        axs[i].set_ylabel(columna)

    #Ocultamos ejes adicionales si el número de gráficos es impar
    if len(lista_columnas_numericas) % 2 != 0:
        axs[-1].axis("off")
    
    plt.tight_layout() #Para que salga limpio
    plt.show()

    return fig, axs #Devuelvo los objetos por si quiero usarlos
...
def categorica_numerica_medida(df, categorical_col, numerical_col, show_values=False, measure='mean'):
...
def categorica_numerica_boxplot(df, cat_col, num_col):
